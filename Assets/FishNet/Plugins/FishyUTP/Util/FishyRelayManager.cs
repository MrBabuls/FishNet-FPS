...
namespace FishNet.Transporting.FishyUTPPlugin
{
    ...
    public class FishyRelayManager : MonoBehaviour
    {
        ...
        [SerializeField] [Tooltip("The join code for the current host, or the code a client should connect to.")]
        ...
        public virtual async void LoginToUnityServices()
        {
            try
            {
                ...
                if (_transport.NetworkManager.CanLog(LoggingType.Common))
                    Debug.Log(
                        $"FishyRelayManager logged into Unity Services. Player ID: {AuthenticationService.Instance.PlayerId}");
            }
            catch (Exception)
            {
